...
if (isset($_POST['add_post'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO business_posts (title, subtitle, category, image_url, author, content) VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssss", $title, $subtitle, $category, $imagePath, $author, $content);
    $stmt->execute();
    header("Location: manage_business_posts.php");
    exit();
}
...
if (isset($_POST['edit_post'])) {
    ...
    $result = $conn->query("SELECT image_url FROM business_posts WHERE id = $id");
    $oldFile = $result->fetch_assoc()['image_url'] ?? null;

    $imagePath = uploadImage("editImageFile", $oldFile);

    $stmt = $conn->prepare("UPDATE business_posts SET title=?, subtitle=?, category=?, image_url=?, author=?, content=? WHERE id=?");
...
if (isset($_GET['delete'])) {
    ...
    $conn->query("DELETE FROM business_posts WHERE id = $id");
    header("Location: manage_business_posts.php");
    exit();
}
...
// --------------------
$postsResult = $conn->query("SELECT * FROM business_posts ORDER BY created_at DESC");
$posts = $postsResult->fetch_all(MYSQLI_ASSOC);

$categoriesResult = $conn->query("SELECT DISTINCT category FROM business_posts WHERE category IS NOT NULL AND category <> ''");
$authorsResult    = $conn->query("SELECT DISTINCT author FROM business_posts WHERE author IS NOT NULL AND author <> ''");
...
<head>
    <meta charset="UTF-8">
    <title>Manage Blog Posts - Business</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-gray-100 min-h-screen">

<header class="bg-red-600 text-white p-4 flex justify-between items-center">
    <h1 class="text-xl font-bold">Manage Blog Posts - Business</h1>
...
<div id="imageModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden">
    ...
    <script>
        ...
        document.addEventListener("click", function(e) {
            ...
            // If clicked on a toggle button or inside dropdown menu → do nothing
